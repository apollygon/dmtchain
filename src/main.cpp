...
#if defined(NDEBUG)
# error "DMTchain cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "DMTchain Signed Message:\n";
...
{
    // DMTchain: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 1 * 60; // DMTchain: 60 seconds
static const int64 nTargetSpacing = 1 * 20; // DMTchain: 20 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // DMTchain: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // DMTchain: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "The Block 18/Jan/2018 UNICEF looks to fund blockchain start-ups";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
...
unsigned char pchMessageStart[4] = { 0xd1, 0xd2, 0xd3, 0xdb }; // DMTchain: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("DMTchainMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("DMTchainMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static DMTchainMiner(CWallet *pwallet)
{
    printf("DMTchainMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("dmtchain-miner");
    ...
    try { loop {
        ...
        printf("Running DMTchainMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("DMTchainMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&DMTchainMiner, pwallet));
}
